...
const CampusReports = () => {
  ...
  useEffect(() => {
    const fetchResults = async () => {
      try {
        setLoading(true);
        setErrorMsg("");
        // Fetch only campus-relevant results
        const practiceRes = await api.get('/campus-admin/student-practice-results');
        const onlineRes = await api.get('/campus-admin/student-online-results');
  ...
  const filteredResults = useMemo(() => {
    return results.filter(result => {
      ...
      return moduleMatch && typeMatch && courseMatch && batchMatch;
    });
  }, [results, filters]);
      ...
      <div className="flex-1 overflow-x-hidden overflow-y-auto">
        <main className="px-6 lg:px-10 py-12">
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
            ...
            <p className="mt-2 text-paragraph">View and analyze results from all student test submissions in your campus.</p>
